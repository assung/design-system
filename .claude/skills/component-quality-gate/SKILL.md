...
### Phase 4.5 — Visual audit(強制 chain,對齊 CLAUDE.md 稽核三級 Tier 1)

**這是 stakeholder-gate,不能跳**。元件 merge 前必過 code + visual 雙層。

1. **Auto invoke**:`npm run visual-audit -- --scope=component:{Name}`(Layer A mechanical:WCAG 對比度 + DOM 幾何 assertion + retina screenshot)
2. Layer A exit code != 0 → 停下,修到 0 violation
3. **Layer B(AI judgement)**:chain `/visual-audit` skill 讀 `snapshots/{Name}-*.png`,做設計合理性判斷(badge 位置、箭頭覆蓋、typography hierarchy、世界級對照)
4. Layer B 若有 finding → 停下修,或在 spec 寫合理例外 rationale(對齊「Consistency Audit 原則」)

**為什麼 mandatory**:code audit 對、spec 對,視覺仍可能錯(DS 歷史反覆出現:DatePicker 四邊不對稱、Badge 疊 Button 位置離譜、Rating 邊框、Carousel 箭頭壓文字...)。merge 前沒過視覺 = 把視覺 bug 帶進 DS。

**合理跳過情境**(極少):純 spec.md 文字修正(無 tsx 改動)→ 視覺無變,可跳 Phase 4.5;但 tsx / token / cva 任一動 → 必跑。
