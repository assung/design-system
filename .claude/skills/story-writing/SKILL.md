...
### Phase 0 — Baseline reference grep(2026-05-20 codify per codex anti-drift D1+D2)

寫 stories wrap **既有 primitive**(`<Sidebar>` / `<ChromeHeader>` / `<Dialog>` / `<DataTable>` / `<Sheet>` 等)前必先:

1. `grep -nE "完整佈局|IconCollapse|WithBulkActions|FullLayout" src/design-system/components/<Primitive>/<primitive>.stories.tsx` 找該 family production-grade「完整佈局」類 story
2. **Read 其 helper**(`WorkspaceBrand` / `UserFooter` / `MAIN_NAV` / `PageContent` / toolbar pattern 等)當 baseline reference
3. 在新 stories.tsx 檔頭標 `// @story-baseline: <path>#<StoryName>` cite reference
4. 禁直接寫 simplified mock(eg. `<SidebarHeader><span>name</span>` / `<SidebarMenuButton><Icon className="size-4">` / `<ChromeHeader><span flex-1>title</span>`)= visual fidelity drift。Hook `check_story_invariants.sh R7` 攔。
