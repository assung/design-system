...
## 連續 item 貼邊合法性(2026-04-22 canonical · v2 精修)

**核心原則**(user 明示):「連續 item 沒有 gap 不合法的情境,是發生在**該 item 與相鄰 item 有可能在視覺上發生底色/邊框相連**的情況。並不是所有 item 都不能相連——會有**視覺上的前提**。」

### 判斷公式(v2)

真 invariant = item **永久視覺層**是否呈現為「**standalone card/pill**」:

```
item 永久 standalone(視覺獨立 card/pill:bg + radius + 不貼父容器邊 inset)
  → 連續排列時必有 gap(防 card 融合破壞各自 identity)

item 永久 flush full-width(貼父容器邊 full-width / 無 per-item radius / 無獨立 bg)
  → 連續排列時 0 gap 合法
  → 分隔 affordance 靠 border-b / bottom progress bar / separator(各 item 自己帶)

item 永久透明 / 有 radius 但只在 hover / selected / focus 瞬時狀態才可見
  → 連續排列時 0 gap 合法(state 視覺稀疏、不會穩定相連)
```

**關鍵細分**:
- **"standalone card/pill"**:item 有 permanent visible container(bg 色塊 / 邊框) + 有 radius + 不貼父容器邊(max-w 或 inset 讓 item 有明顯的 card 輪廓)
- **"flush full-width"**:item 貼父容器邊、無 per-item radius、bg 透明或繼承父層。Row 之間用 border-b / 底部線條 / separator 作**分隔 affordance**——這屬於「分隔線型視覺」,非「card 容器型視覺」
- **"state 視覺"**:hover bg / selected bg / focus ring / active bg—— 這些是瞬時或稀疏的 state-triggered 視覺,不屬於 permanent layer

**為什麼「有 radius ≠ 必 gap」**(M12 counter-example avoidance):
- M3 Navigation drawer 每 item 有 radius 但 item 永久透明(radius 只在 selected state 可見) → 連續仍 0 gap 合法
- Apple Inset Grouped list:group 層有 radius(僅首尾 row 繼承) / row 之間 permanent transparent + separator → group 內部 0 gap
- 故「radius」本身非 trigger,「permanent standalone card」才是

### 世界級 benchmark(2026-04-22 掃 6 家)

| DS | 情境 A menu item | 情境 B file/card list | 情境 C table row | 情境 D settings row |
|----|------------------|----------------------|------------------|---------------------|
| Polaris(Shopify)| flush + border-b,0 gap | ResourceList flush rows border-b;MediaCard standalone 才 card | IndexTable flush border-b 0 gap | Card.Section 內 Divider 0 gap |
| Material M3 | flush no-radius per item,0 gap | Card `variant="outlined"` standalone 多 card `gap 8–16dp` | DataTable flush border-b 0 gap | Nav drawer:radius + **0 gap**(透明 bg)|
| Atlassian | Menu flush 0 gap | `<Stack space="space.100">` 8px for standalone cards | DynamicTable flush border-b 0 gap | Nav rounded variant(透明 bg) 0 gap |
| Ant Design | default flush 0 gap;v5 Menu itemMarginBlock 4px 是 inset rounded 特別 variant | Upload picture-card **margin 8px**;text / picture 為 flush 0 gap | Table flush border-b 0 gap | List bordered flush + divider,0 gap |
| Carbon(IBM)| flush + 1px border-b,0 gap | FileUploaderItem flush 0 gap border-b | DataTable flush border-b 0 gap | Side-nav flush 0 gap |
| Apple HIG | N/A | Inset Grouped:group radius + **group 內 rows 0 gap**,group **之間** gap | N/A | Settings Inset Grouped 同左 |

**共識**:default flush row(menu / table / file / settings)= **flush + no per-item radius + 0 gap + separator**。只有升級為 standalone card / pill stack 才需 gap(Material Card stack / Ant Upload picture-card / Atlassian `<Stack>`)。

### 對照表(v2,對齊精修公式)

| Item 類型 | 永久視覺層分類 | 連續排列 gap | 合法依據 |
|-----------|--------------|-------------|---------|
| **MenuItem / Select item / DropdownMenuItem / SidebarMenuButton / TreeItem** | 永久透明(radius 只在 hover 可見) | **0 gap 合法** | state 視覺稀疏——同一瞬間最多一個 item 在 hover |
| **SelectionItem**(Radio / Checkbox row) | **永久透明**(code 驗證:`flex items-start gap-2`,無 bg/border/radius) | **0 gap 合法** | 選中態用 control 視覺(勾 / radio 點)非 row bg |
| **DataTable row** | flush full-width + border-b 分隔線 | **0 gap 合法** | 「分隔線型視覺」— border-b 本身是 separator affordance,非 card 容器 |
| **FileItem compact + progress bar**(Type A upload manager) | 永久透明 row + 底部 progress bar(≈ border-b 型 affordance) | **0 gap 合法** | 同 DataTable 邏輯:progress bar 是分隔線型 affordance,非 card 容器(user 2026-04-22 確認) |
| **FileItem rich**(永遠 card) | permanent `border + bg-surface + rounded-md + inset(max-w-md 不貼父邊)`→ **standalone card** | **必 gap ≥ 8px** | Card 邊框會相黏融合成單一 card,破壞各自 identity |
| **FileItem compact + bg-neutral-3**(Type B form attachment 靜態) | permanent `bg-neutral-3 + rounded-md + inset`→ **standalone pill** | **必 gap ≥ 4px** | 連續 bg 塊會合成一塊大 bg,item 邊界消失 |
| **StepItem / Timeline item**(horizontal 或 vertical) | 永久透明 item + connector line(連接線)affordance | **0 gap 合法**(gap 由 connector own) | connector pattern:item 之間的視覺連接由 line 主動提供,非 gap。類似 DataTable border-b |

### 狀態視覺(hover / focus / selected / active)的處理

State 視覺**不屬於永久視覺層**,不觸發本規則:

| State | 特性 | 為什麼不觸發 gap rule |
|-------|------|-------------------|
| **hover** | 瞬時 + 單一 | 同一時間最多一個 item 在 hover,不可能兩個相連 |
| **focus-visible** | 瞬時 + 單一 | 同上(鍵盤焦點唯一) |
| **active / pressed** | 瞬時 + 單一 | 同上(mousedown 瞬間) |
| **selected** | 半永久 + 可能連續 | **需另外判斷**:multi-select 的連續 selected bg 視覺連續是 **feature**(表達「選了一組連續 item」),非 bug。除非 item 永久是 standalone card(FileItem rich selected)則必 gap |

### 新 item 元件 checklist

建新 row / item 類元件時,開 spec 必自問:

1. 本元件**永久**(非 state)視覺是:(a) standalone card/pill(有 bg + radius + 不貼父邊) / (b) flush full-width / (c) 完全透明?
2. 若 (a) → spec 必明寫「list wrapper 必 gap ≥ Xpx」canonical + rationale
3. 若 (b) 或 (c) → spec 可明寫「list wrapper 可 0 gap」+(b) 列出 separator affordance 來源

### 反例(本 session 2026-04-22 修正)

- `file-upload.stories.tsx` compact list:FileItem compact 靜態(Type B)有 `bg-neutral-3 rounded-md inset` = standalone pill → 改 `gap-1`
- `file-upload.stories.tsx` rich list:FileItem rich = standalone card → 改 `gap-2` + 移除外框
- `file-item.stories.tsx` Rich / HoverSwap / Clickable:同上
