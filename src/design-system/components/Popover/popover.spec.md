...
**Close X 按鈕(2026-04-20 決策,2026-04-22 size 精緻化)**:所有 `PopoverHeader` 內建右上 X 按鈕(對齊 Dialog canonical)—— consumer 只需渲染 `<PopoverHeader>title 內容</PopoverHeader>`,X 自動出現並綁 Radix Close。實作 = `<Button iconOnly dismiss size="xs" startIcon={X} aria-label="關閉" />`(非 Inline Action / 非自刻 button;詳見 `patterns/element-anatomy/item-anatomy.spec.md`「Dismiss canonical」+ `patterns/overlay-surface/overlay-surface.spec.md`「Close X canonical」)。有 header 的 Popover 通常結構化程度高(title / 多區塊),明確的關閉入口讓使用者更易退出。無 header 的簡單 Popover 不加 X(click-outside / Esc 即可)——`hideClose` prop 可讓 composition 元件(如 Coachmark)選擇隱藏。

**Header chrome size canonical(Popover 特化,2026-04-22)**:PopoverTitle 用 `text-body`(14px)、chrome buttons(close X 與其他 header action)**預設 `size="xs"`**,**非** Dialog 的 body-lg + sm。Rationale:
- Popover 是 **non-modal 輕量浮層**(density 鎖 md、chrome 不隨頁面密度放大);整個 header 視覺包(title 字級 + button chrome)都走「輕量」,跟 Dialog(modal 重量級 = title body-lg + chrome sm)形成明確區分
- 世界級對照:Notion / Linear / Figma / Stripe 的 popover / filter panel / inline settings 的 close X 都比 modal dialog 小一號,用「視覺重量」區分「必處理 vs 可忽略」
- **Consumer rule**:Popover header 內的 action button 預設 `size="xs"`;要放 sm/md 視為違反 chrome 輕量原則,需 rationale 才允許偏離
