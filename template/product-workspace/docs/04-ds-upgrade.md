...
`@your-org/design-system` 改版時,product-workspace **自動跟上**的流程。
...
| **Renovate(`renovate.json`)** | 每週一 6am 掃 `@your-org/*` 新版 → 開 PR(label `ds-bump`)|
| **`.github/workflows/audit.yml`** | Renovate PR 自動跑 audit → 全綠才能 merge |
| **Pre-release `@beta` dist-tag** | DS 端 ship beta 後 consumer 可手動 `npm install @your-org/design-system@beta` 試 |
...
DS repo:dev push → tag v0.2.0 → CI publish npm
  ↓
npm registry:@your-org/design-system@0.2.0 available
  ↓
product-workspace renovate(週一 6am):
  - 開 PR "chore(deps): bump @your-org/design-system to 0.2.0"
...
- 新元件 consumer 不用動就可 `import { NewComponent } from '@your-org/design-system'`
...
Steps:
1. DS owner 寫好 codemod 在 `@your-org/design-system/codemods/v0-to-v1/`
2. Consumer 跑:
   ```bash
...
```bash
# 試 beta
npm install @your-org/design-system@beta --save

# 試 next(下一 major 預發)
npm install @your-org/design-system@next --save

# Pin specific pre-release
npm install @your-org/design-system@0.2.0-beta.3 --save-exact
```
...
```bash
npm view @your-org/design-system versions    # 全版本
npx changelog @your-org/design-system        # 詳細 changelog(per changesets)
```

Or browse `your-org/design-system` GitHub Releases。

## 不要做的

- ❌ 改 `node_modules/@your-org/design-system/` 的 .tsx — 下次 install 被覆蓋
- ❌ Pin 死版本(`"@your-org/design-system": "0.1.0"`)— 失去 auto-update,security patch 收不到
