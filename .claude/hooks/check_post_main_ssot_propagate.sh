...
SSOT_DIFF=$(git diff --name-only HEAD~1 HEAD 2>/dev/null | grep -E '^(packages/(design-system|storybook-config)/src/|\.claude/(rules|hooks|skills|commands|references)/|\.claude-plugin/(plugin|marketplace)\.json|hooks/hooks\.json|CLAUDE\.md)' || true)
