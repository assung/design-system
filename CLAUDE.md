...
| **M11** | **User-perspective interactive state walk — 改完 UI 後 present 前必親自走一次 user 視角**,不留待 user 抓。改 UI 完成必過 7 題 self-test:(a) **static**(對齊 / padding / 色);(b) **hover / selected / active**(**真實 invariant = content 必在 affordance bg 內有 padding**:text / icon / avatar / control 等 content 元素**不可直接觸到 hover bg / selected bg 邊緣**,bg 寬度必比 content 寬,content 在 bg 內有 breathing(通常 8px);世界級 Material M3 / Polaris / Linear / Slack / Notion / GitHub Primer list row 全部 content-inside-bg;bg 邊位置(flush body padded 內邊 / inset body)**有設計自由**是 variance 不是 canonical — 依 DS 一致性選;鼠標 cursor;hover 區覆蓋整列。詳 `patterns/overlay-surface/overlay-surface.spec.md` 規則 3.1);(c) **focus-visible**(只 keyboard tab 顯示 ring,滑鼠 click 不該 — 檢查 `focus:` vs `focus-visible:` 用對);(d) **active / pressed**;(e) **keyboard**(Tab / Shift+Tab / Esc / Enter 正確);(f) **範例真實性**(「誰會這樣用?」— modal 內 list item 通常 in-modal 直接設定,不 navigate 去別處;symbolic tags / placeholder content 全改掉);(g) **CSS 對稱**(`lg` override 必對應 `md` reset;`dark` 必對應 `light` default)。每 UI 改動未跑 7 題 walk 就 commit = 違反本條。 | 2026-04-22 ListBody 修完 user 連抓 5 波:hover bg 貼邊違反不貼邊 / focus ring click 觸發擾人 / notification 範例不現實(誰會進 modal 又跳別處)/ menu py-2 沒對齊 / layoutSpace md reset selector 缺。每個都是 user 視角一看就知,AI 沒跑 state walk 就 commit |
