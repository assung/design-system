...
## 24px 閾值對齊規則(Prefix 與 Suffix 共用同一條公式) → `.claude/references/item-anatomy-24px-threshold.md`

**核心原則**:每個 slot 的對齊容器高度由**自己內容物的大小**決定;Prefix 和 suffix 用同一條公式但各自獨立判斷。

**公式**:內容 ≤ 24 → `h-[1lh]`(對齊 label 第一行);> 24 + 有 desc → block `h-[calc(1lh + 2px + desc_1lh)]`(對齊文字塊中心);> 24 無 desc → 強制 inline。

**關鍵規則**:prefix / suffix 各自獨立對齊(不互相同步);`<ItemAvatar>` / `<ItemIcon>` helper 從 `RowSizeContext` 自動查表,禁止硬寫 size;asChild consumer 用 helper 零尺寸責任;uniform prefix slot 由 `:has()` selector 全域對齊(opt-in)。

**完整公式表 / Avatar size table / 業界 convention / asChild pattern / uniform prefix 詳例 / 禁止事項** → 見 reference。
