...
`naked` = cell-as-input substrate(Notion / Airtable / Excel / AG Grid:host cell 提供 box,Field 融入)。**核心原則(2026-05-05 v9 architectural rewrite)**:naked 完全繼承 Field default state machine,**只改物理尺寸**(rounded-none / h-full / cell padding tokens)。User 原話:「原本 field 的互動樣式都能保留,唯一差別就是 field 會填滿 cell,然後沒有圓角而已」。

| Layer | Home | 內容 |
|---|---|---|
| **L1 Slot 幾何** | `patterns/element-anatomy/item-anatomy.tsx` | `<ItemPrefix>` / `<ItemSuffix>`(永遠 `h-[1lh]` 對齊第 1 行,單行視覺=items-center)。startIcon / value-icon 用 ItemPrefix;chevron / clear / calendar / endAction / endIcon 用 ItemSuffix |
| **L2 Wrapper align** | `field-wrapper.tsx` `nakedCellRowModeAlign` | host cell `data-row-mode` propagate(autoRow→items-start / fixed→items-center)|
| **L3 State machine** | **完全繼承 Field default**(border-based)| edit:`border-border` rest / `hover:border-border-hover` / `focus-within:border-primary focus-within:hover:border-primary` / `data-[state=open]:border-border-hover`。display/readonly/disabled:`border-transparent` 預留 1px 防 layout shift |
| **L4 Display hover** | `field-wrapper.tsx` `nakedCellEditableDisplayHover` | editable cell display mode 唯一 outline(對齊 Notion/Airtable hover-cell-shows-border) — `outline-1 outline-offset-[-1px] outline-[var(--border-hover)]` |

**前身已 retire**:
- v4-v8 `outline-2 outline-offset-[-1px] outline-{border\|primary}` 平行 state machine(thickness 加粗 / hover 蓋 focus / open 藍框 vs Field default open hover-color drift)
- `nakedCellHoverRing` / `nakedCellFocusRing` / `nakedCellOpenRing` 三 const(已下沉到 Field default state machine)
- `nakedCellPrefixSlot` / `nakedCellSuffixSlot`(已下沉到 L1 `<ItemPrefix>` / `<ItemSuffix>`)

**反模式 ❌**:naked variant 自寫 `outline-*` / `box-shadow inset` state ring(用 L3 Field default 繼承)/ hardcode `<span class="h-[1lh]...">` slot(用 L1)/ hardcode `items-center` wrapper(用 L2)。

**機械防漂移**:hook `check_naked_row_mode_propagation` / `check_field_state_token_consume`(rule 已 v9 升級:禁 outline-{border\|primary} 自寫)/ `check_inline_action_canonical_gap` / `check_row_slot_handcraft`(write-time)+ audit `/design-system-audit` Group N M36-M39(periodic)。
