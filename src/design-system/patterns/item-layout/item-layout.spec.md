...
此規則跨元件共用（Field、SelectMenu、DropdownMenu）：

| Icon 角色 | 顏色 | 範例 |
|---|---|---|
| 描述操作/內容的 icon（prefix） | `fg-muted` | Trash2, Settings, Mail |
| 指示方向/層級的 icon（suffix） | `fg-muted` | ChevronRight, ExternalLink, ChevronDown |
| 代表狀態值的文字（suffix value） | `fg-muted` | "Dark", "已啟用" |
| disabled 時所有元素 | `fg-disabled` | 全部統一 |

**判斷標準：「這個 icon 是描述內容還是指示方向？」——不管答案是哪個，都是 `fg-muted`。** icon 在 item 中永遠是輔助角色，不應搶過 label 的注意力。
