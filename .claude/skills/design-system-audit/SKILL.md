...
# Design System Audit (64 dimensions, Groups A–P + Future-proof preflight)
...
| 64 | **/knowledge-prune Phase Z cross-repo SSOT propagation**(2026-05-26 codify per user verbatim「我如果在 ds repo 跑 knowledge audit deep,完成後 product workspace repo 是否能主動更新同步?照理說應該要可以才對」)| Audit knowledge-prune/SKILL.md「Phase Z」段:(a) detect SSOT-affecting changes(`packages/design-system/src` + `.claude/{rules,hooks,skills,commands,references}` + `CLAUDE.md` + `hooks/hooks.json` + `.claude-plugin`);(b) trigger pipeline:npm version bump → tag push → Release workflow → npm publish → Dependabot 24h 內 auto-PR product-workspace + fork repos;(c) Phase Z user-facing summary 含「✅ Phase Z triggered v0.1.0-beta.N+1 → Dependabot will auto-PR」or「⏸ 無 SSOT-affecting 跳過」。對應「一次更新所有 DS repo 增刪改」directive,不需手動 1 個 1 個 propagate |
