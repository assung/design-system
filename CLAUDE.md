...
| **M16** | **訂 standalone card/pill 容器 canonical 必同步訂 multi-instance gap canonical**。任何元件的**永久視覺層**呈現為 **standalone card/pill**(有 bg + radius + 不貼父容器邊 inset)時,**必**同時在 spec 訂「多個連續排列時的最小 gap」canonical — 因為 consumer 在 stories / product 寫 `.map()` 時極易漏 gap 造成 card 融合 / bg 塊相連(破壞各自 identity)。**核心公式**(item-anatomy SSOT v2「連續 item 貼邊合法性」):`permanent standalone card/pill → 必 gap;permanent flush full-width(貼邊無 radius)/ permanent transparent(radius 只在 state 可見) → 0 gap 合法,分隔靠 border-b / progress bar / separator`。**不 trigger 的情況**:item 有 radius 但 bg 透明(state 才可見,如 M3 Nav drawer、Apple Inset Grouped row);item flush full-width 用 border-b(如 DataTable / Menu item)。**違反 trigger**:consumer stories / product code 連續 N 個 standalone card/pill 視覺相連 = spec 缺 list-gap canonical,M16 violation。**實作**:(1)元件 spec 加「List wrapper canonical」節,列 gap 值 + rationale + 反例;(2)hook `check_item_list_gap.sh` grep consumer `map()` wrapper 無 gap 或加外框 → block;(3)audit Dim 加「consumer 層 list wrapper 是否正確消費 item gap canonical」。**世界級 benchmark 支持**:2026-04-22 掃 Polaris / Material M3 / Atlassian / Ant / Carbon / Apple HIG 6 家共識 — default flush row 0 gap + separator;standalone card stack 才需 gap(見 item-anatomy.spec.md「世界級 benchmark」表)。 | 2026-04-22 FileItem rich `border card` + compact `bg-neutral-3` 先前已訂 canonical,但 spec 只寫**單 item 視覺**沒寫**連續 item gap**,導致 file-upload.stories.tsx rich list 加 `border rounded-lg overflow-hidden` 強制邊框相黏、compact list 無 gap bg 塊連一大片。User 貼圖糾正才發現。若 M16 存在,訂 AR17/AR20 canonical 時同步訂 list-gap,hook 立刻攔截。v2 精修觸發條件避免 DataTable / MenuItem hover-only radius 被誤判 |
