...
**專案最常發生的錯誤是「AI 憑直覺造新 pattern」(延伸 mindset #1)。** 遇到無明確前例的設計決策或實作選擇時,**強制按以下順序**處理,禁止跳步、禁止憑感覺往前:
...
- ❌ 憑直覺命名(見 mindset #1「不憑直覺發明」)——必先 `ls` / `grep` 既有 pattern
...
**寫任何 gap、padding、font-size、line-height、icon size、border-radius 等數值之前,必須先 grep 系統內同類型的值,確認是否有既有 pattern 可以直接套用(延伸 mindset #1「不憑直覺發明」+ 本章下方自我檢查)。**
