...
const meta: Meta = {
  title: 'Design System/Components/Fields/LinkField/設計規格',
    ...
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          <H3>結構（Anatomy）</H3>
          <Desc>LinkField 是 URL 輸入與顯示元件。edit 模式在「link 狀態」與「input 狀態」之間切換——有合法 URL 時顯示藍色連結 + Pencil 編輯按鈕；無值或正在編輯時顯示文字輸入框。</Desc>
        </div>
          ...
          <div className="px-10 py-8 rounded-lg bg-canvas border border-divider flex items-center justify-center">
            <div className="w-64">
              {mode === 'edit' && editState === 'link' && (
                <LinkField mode="edit" size={size} value="https://github.com" onChange={() => {}} />
              )}
              {mode === 'edit' && editState === 'input' && (
                <LinkField mode="edit" size={size} value="" onChange={() => {}} />
              )}
              {mode === 'edit' && editState === 'error' && (
                <LinkField mode="edit" size={size} value="" onChange={() => {}} error />
              )}
              {mode === 'readonly' && (
                <LinkField mode="readonly" size={size} value="https://github.com" />
              )}
              {mode === 'disabled' && (
                <LinkField mode="disabled" size={size} value="https://github.com" />
              )}
            </div>
          </div>
...
export const Inspector = {
  ...
  render: () => (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        <H3>元件檢閱器</H3>
        <Desc>選擇 mode / state / size 組合，即時查看所有 token。LinkField 的 edit mode 有三種內部狀態：link（藍色連結 + Pencil）、input（文字輸入框）、error（紅框）。</Desc>
      </div>
      <InspectorInner />
    </div>
  ),
  ...
  render: () => (
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-1">
        ...
        <Desc>LinkField 的色彩設計重點：edit-link 狀態的 value 用 text-primary 呈現為可點擊連結；error 狀態的邊框用 border-error。色塊即時渲染，切 dark mode 自動更新。</Desc>
      </div>
                ...
                return (
                  <tr key={st}>
                    ...
                    <td className="p-3 border-b border-divider min-w-[200px]">
                      {st === 'link' && <LinkField mode="edit" value="https://github.com" onChange={() => {}} />}
                      {st === 'input' && <LinkField mode="edit" value="" onChange={() => {}} />}
                      {st === 'error' && <LinkField mode="edit" value="" onChange={() => {}} error />}
                    </td>
              ...
              {(['readonly', 'disabled'] as const).map((m) => {
                ...
                return (
                  <tr key={m}>
                    <Td mono>{m}</Td>
                    <td className="p-3 border-b border-divider min-w-[200px]">
                      <LinkField mode={m} value="https://github.com" />
                    </td>
                    <td className="p-3 border-b border-divider align-top">
                      <TokenAnnotation colors={c.wrapper} />
                    </td>
        ...
        <div className="flex flex-col gap-3">
          {SIZES.map((sz) => (
            <div key={sz} className="flex items-center gap-4">
              ...
              <LinkField mode="edit" size={sz} value="https://github.com" onChange={() => {}} className="max-w-xs" />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3">
          {SIZES.map((sz) => (
            <div key={sz} className="flex items-center gap-4">
              <span className="text-[11px] text-fg-muted w-8 shrink-0 font-mono">{sz}</span>
              <LinkField mode="edit" size={sz} value="" onChange={() => {}} className="max-w-xs" />
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-col gap-3">
          {(['readonly', 'disabled'] as const).map((m) => (
            <div key={m} className="flex items-center gap-4">
              ...
              <LinkField mode={m} value="https://github.com" className="max-w-xs" />
            </div>
          ))}
        </div>
      </div>
