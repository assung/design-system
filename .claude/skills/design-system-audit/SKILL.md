...
| 37 | **Naked variant Field default state machine 繼承**(M19,2026-05-05 v9 architectural rewrite)| DS-wide grep cell-as-input naked variant 元件,確認 state(hover/focus/open)**完全繼承 Field default border-based state machine**(`border-border-hover` / `border-primary` / `focus-within:hover:border-primary` / `data-[state=open]:border-border-hover`),**禁自寫平行 outline state ring**(前 v4-v8 `outline-2 outline-offset-[-1px] outline-{border\|primary}` 已 retire — thickness 加粗 + hover 蓋 focus + open 藍框 vs Field default open hover-color 多重 drift)。Hook `check_field_state_token_consume.sh` v9 升級規則 write-time 攔。違反 = visual diverge from Field default + Field 三變體 token 改動不同步 |
