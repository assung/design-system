...
實作(v5 — 派 B,inspired by Material OutlinedInput):

```tsx
// FCG outer 自帶 frame(border + rounded + overflow-hidden)
'border border-border rounded-md overflow-hidden bg-surface'
'hover:border-border-hover'
'has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-focus-ring has-[:focus-visible]:border-border-hover'
// Cells 內部 flat(strip own border + rounded,保留 own bg)
'[&>*]:!border-0 [&>*]:!rounded-none'
// Internal divider via ::before pseudo on cells[2..]
"[&>*+*]:relative [&>*+*]:before:absolute [&>*+*]:before:left-0 [&>*+*]:before:top-0 [&>*+*]:before:bottom-0 [&>*+*]:before:w-px [&>*+*]:before:bg-border [&>*+*]:before:content-['']"
// Cell error state via box-shadow inset(避免跟 outer frame 衝突)
'[&>*[data-error]]:shadow-[inset_0_0_0_1px_var(--error)]'
```

**為什麼派 B(2026-05-04 重構)**:派 A `-ml-px` overlap 模式下,disabled cell 灰底跟 edit cell 白底會在 boundary 產生「bg 色差視覺分隔線」,加上 cell 自帶 border 會雙重壓深感。派 B 由 outer frame 主導 group integrity,cells 沒有彼此 border overlap,divider 改 ::before pseudo 1px line(均勻無壓深)。
- ✓ Outer frame:border + rounded(group-level signal)
- ✓ Cells 保留 own bg:edit white / disabled gray / Button primary 等都自然 work
- ✓ Hover/focus 改 group-level signal(整 group highlight)— 對齊 Material OutlinedInput idiom
- ✓ Error state 仍 cell-level(box-shadow inset)— 單 cell error 也能精確標示

**Source(world-class precedent)**:
- Material OutlinedInput NotchedOutlineRoot — `github.com/mui/material-ui/blob/v6.0.0/packages/mui-material/src/OutlinedInput/OutlinedInput.js`(absolute-positioned outer frame overlay)
- Pattern adapted to multi-field compose;direct copy 不 applicable(Material 用於 single-field-with-adornments),但 architecture inspired by
