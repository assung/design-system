...
const meta: Meta = {
  title: 'Design System/Components/Fields/MultiSelectField/設計規格',
...
/** Mode x State token map — traced from field-wrapper.tsx cva + multi-select-field.tsx */
...
/** Error state overrides (edit mode only) — traced from multi-select-field.tsx error classes */
                ...
                {(['readonly', 'disabled'] as const).map((m) => (
                  <tr key={m}>
                    ...
                    <td className="p-3 border-b border-divider align-top min-w-[180px]">
                      <MultiSelectField mode={m} options={categoryOptions} value={['electronics', 'food']} size="sm" />
                    </td>
                    <td className="p-3 border-b border-divider align-top min-w-[160px]">
                      <MultiSelectField mode={m} options={categoryOptions} value={[]} size="sm" />
                    </td>
                    <td className="p-3 border-b border-divider align-top min-w-[160px]">
    ...
    <div className="flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-3">
          {SIZES.map((sz) => (
            <div key={sz} className="flex items-center gap-3">
              <MultiSelectField size={sz} options={categoryOptions} value={['electronics', 'food']} onChange={() => {}} className="w-64" />
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3">
          {SIZES.map((sz) => (
            <div key={sz} className="flex items-center gap-3">
              <MultiSelectField mode="readonly" size={sz} options={categoryOptions} value={['electronics', 'food']} className="w-64" />
              <span className="text-caption text-fg-muted font-mono">size=&quot;{sz}&quot;</span>
            </div>
          ))}
        </div>
  ...
  render: () => {
    ...
    return (
      <div className="flex flex-col gap-8">
        <div className="flex flex-col gap-1">
          <H3>狀態行為</H3>
          <Desc>MultiSelectField 特有的溢出、換行、清除、個別移除行為。</Desc>
        </div>
